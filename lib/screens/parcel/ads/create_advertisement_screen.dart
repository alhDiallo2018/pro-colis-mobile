...
class CreateAdvertisementScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CreateAdvertisementScreen> createState() =>
      _CreateAdvertisementScreenState();
}

class _CreateAdvertisementScreenState
    extends ConsumerState<CreateAdvertisementScreen> {
  ...
  Future<String?> _getVoicePath() async {
    try {
      ...
      return '${dir.path}/ad_voice_$ts.m4a';
    } catch (e) {
      debugPrint('Erreur chemin audio annonce: $e');
      return null;
    }
  }
  ...
  Future<void> _stopRecording() async {
    try {
      ...
      debugPrint('Erreur arrêt enregistrement annonce: $e');
  ...
  Future<void> _playVoice(VoiceMessage message) async {
    try {
      ...
      debugPrint('Erreur lecture audio annonce: $e');
  ...
  void _deleteLocalFile(String path) {
    try {
      ...
      debugPrint('Erreur suppression fichier annonce: $e');
    }
  }
    ...
    try {
      ...
      for (final msg in _voiceMessages) {
        try {
          ...
          debugPrint('Erreur upload note vocale annonce: $e');
        }
      }
      ...
      final result = await _apiService.createAdvertisement(data);
      ...
      if (result['success'] == true || result['id'] != null) {
        ...
        _showSnack(
          result['message']?.toString() ?? 'Erreur lors de la publication',
          isError: true,
        );
      }
    } catch (e) {
      debugPrint('Erreur publication annonce: $e');
      if (mounted) _showSnack('Erreur lors de la publication', isError: true);
    } finally {
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 2),
                Text(
                  'Les clients sur votre itinéraire pourront vous proposer leurs colis.',
                  style: TextStyle(
                    color: Colors.white,
                    fontSize: 12.5,
                    height: 1.4,
        ...
        DropdownButtonFormField<Garage>(
          ...
          items: _departureOptions.map((g) {
            return DropdownMenuItem<Garage>(
              value: g,
              child: Text('${g.name} · ${g.city}', overflow: TextOverflow.ellipsis),
            );
          }).toList(),
          onChanged: (value) {
